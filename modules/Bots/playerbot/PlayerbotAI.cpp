...
uint32 PlayerbotChatHandler::extractQuestId(string str)
{
	...
	const char* typeMark = "Hquest";
	char* cId = extractKeyFromLink(source, typeMark);
...
namespace MaNGOS
{
    ...
    {
    public:
        UnitByGuidInRangeCheck(WorldObject const* obj, uint64 guid, float range) : i_obj(obj), i_range(range), i_guid(guid) {}
    ...
    {
    public:
        GameObjectByGuidInRangeCheck(WorldObject const* obj, uint64 guid, float range) : i_obj(obj), i_range(range), i_guid(guid) {}
        WorldObject const& GetFocusObject() const { return *i_obj; }
...
Unit* PlayerbotAI::GetUnit(uint64 guid)
{
    if (!guid)
        return NULL;
...
Creature* PlayerbotAI::GetCreature(uint64 guid)
{
    if (!guid)
        return NULL;
...
GameObject* PlayerbotAI::GetGameObject(uint64 guid)
{
    if (!guid)
        return NULL;
...
bool PlayerbotAI::TellMaster(string text, PlayerbotSecurityLevel securityLevel)
{
    ...
    if (!bot->isMoving() && !bot->IsInCombat() && bot->GetMapId() == master->GetMapId() && !bot->IsTaxi())
    {
...
bool PlayerbotAI::CastSpell(uint32 spellId, Unit* target)
{
	...
	if (pet && pet->HasSpell(spellId))
    {
		...
		pet->ToggleAutocast(pSpellInfo, !autocast);
		ostringstream out;
		out << (autocast ? "|cffff0000|Disabling" : "|cFF00ff00|Enabling") << " pet auto-cast for ";
		out << chatHelper.formatSpell(pSpellEntry);
        TellMaster(out);
        return true;
    }
    ...
    else if (pSpellInfo->Targets & TARGET_FLAG_DEST_LOCATION)
    {
		...
		targets.SetDst(aoe.GetPositionX(), aoe.GetPositionY(), aoe.GetPositionZ(), aoe.GetOrientation(), aoe.GetMapId());
    }
    else if (pSpellInfo->Targets & TARGET_FLAG_SOURCE_LOCATION)
    {
		Position destPosition;
		bot->GetPosition(&destPosition);
    ...
    if (!bot->isInFront(faceTo, M_PI / 2) && !bot->IsTaxi())
    {
...
{
    const SpellInfo* const pSpellInfo = spell->m_spellInfo;
