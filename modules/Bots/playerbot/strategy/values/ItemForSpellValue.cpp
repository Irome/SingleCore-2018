...
Item* ItemForSpellValue::Calculate()
{
    ...
    if (trader)
    {
        ...
        if (itemForSpell && itemForSpell->IsFitToSpellRequirements(pSpellInfo))
            return itemForSpell;
    }
...
Item* ItemForSpellValue::GetItemFitsToSpellRequirements(uint8 slot, SpellEntry const *spellInfo)
{
	...
	const SpellInfo* pSpellInfo = sSpellMgr->GetSpellInfo(spellInfo->Id);

	if (itemForSpell->IsFitToSpellRequirements(pSpellInfo))
		return itemForSpell;
