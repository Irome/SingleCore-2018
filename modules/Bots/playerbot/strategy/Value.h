...
namespace ai
{
    ...
    class ObjectGuidListCalculatedValue : public CalculatedValue<list<uint64> >
    {
    public:
        ObjectGuidListCalculatedValue(PlayerbotAI* ai, string name = "value", int checkInterval = 1) :
            CalculatedValue<list<uint64> >(ai, name, checkInterval) {}

        virtual string Format()
        {
            ostringstream out; out << "{";
            list<uint64> guids = Calculate();
            for (list<uint64>::iterator i = guids.begin(); i != guids.end(); ++i)
            {
                uint64 guid = *i;
                out << guid << ",";
            }
