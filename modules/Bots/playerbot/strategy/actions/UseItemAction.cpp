...
bool UseItemAction::Execute(Event event)
{
    ...
    list<uint64> gos = chat->parseGameobjects(name);
...
bool UseItemAction::UseGameObject(uint64 guid)
{
...
{
    return UseItem(item, 0, NULL);
}

bool UseItemAction::UseItemOnGameObject(Item* item, uint64 go)
{
...
{
    return UseItem(item, 0, itemTarget);
}

bool UseItemAction::UseItem(Item* item, uint64 goGuid, Item* itemTarget)
{
    ...
    *packet << bagIndex << slot << cast_count;
    ...
    if (goGuid)
    {
        ...
        if (go && go->isSpawned())
        {
            uint16 targetFlag = TARGET_FLAG_GAMEOBJECT;
            *packet << targetFlag;
            packet->appendPackGUID(goGuid);
	...
	if (!targetSelected && item->GetTemplate()->Class != ITEM_CLASS_CONSUMABLE && master)
	{
		...
		if (masterSelection)
		{
			Unit* unit = ai->GetUnit(masterSelection->GetGUID());
			if (unit)
			{
    ...
    for (int i=0; i<MAX_ITEM_PROTO_SPELLS; i++)
    {
        ...
        {
            *packet << (uint16)TARGET_FLAG_NONE ;
            targetSelected = true;
            out << " on self";
        }
        break;
