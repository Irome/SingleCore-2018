...
bool TrainerAction::Execute(Event event)
{
	...
	{
		creatureGUID = masterSelectedUnit->GetGUID();
	}
    Creature *creature = ai->GetCreature(creatureGUID);
    if (!creature)
        return false;

	if (!creature->isCanTrainingAndResetTalentsOf(bot))
	{
		return false;
	}

    // check present spell in trainer spell list
    TrainerSpellData const* allSpells = creature->GetTrainerSpells();
    if (!allSpells)
    {
