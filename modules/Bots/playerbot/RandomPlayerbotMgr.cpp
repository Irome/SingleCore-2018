...
void RandomPlayerbotMgr::RandomTeleport(Player* bot)
{
	...
	CellCoord p(Trinity::ComputeCellCoord(bot->GetPositionX(), bot->GetPositionY()));
	Cell cell(p);
	cell.SetNoCreate();
	...
	cell.Visit(p, world_unit_searcher, *bot->GetMap(), *bot, range);

    for(list<Unit *>::iterator i = targets.begin(); i!= targets.end(); ++i)
    {
        Unit* unit = *i;
        WorldLocation loc;
		loc = unit->GetWorldLocation();
        locs.push_back(loc);
    }
