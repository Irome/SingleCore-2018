...
{
    sLog->outString( "Creating new random bot for class %d", cls);
    ...
    sLog->outString( "Random bot created for account %d - name: \"%s\"; race: %u; class: %u",
            accountId, name.c_str(), race, cls);
...
void RandomPlayerbotFactory::CreateRandomBots()
{
    ...
    for (int accountNumber = 0; accountNumber < sPlayerbotAIConfig.randomBotAccountCount; ++accountNumber)
    {
        ...
        sLog->outString("Account %s created for random bots", accountName.c_str());
    }
    ...
    for (int accountNumber = 0; accountNumber < sPlayerbotAIConfig.randomBotAccountCount; ++accountNumber)
    {
		...
		sLog->outString("%d Characters for account %d ", count, accountId);
		...
		for (uint8 cls = CLASS_WARRIOR; cls < MAX_CLASSES; ++cls)
		{
			if (cls != 10 && cls != CLASS_DEATH_KNIGHT)
			{
				...
				//factory.CreateRandomBot(CLASS_HUNTER);
			}
		}
