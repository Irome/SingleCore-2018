...
void PlayerbotFactory::InitPet()
{
	...
	if (!pet)
	{
		...
		CreatureTemplate const* co = sObjectMgr->GetCreatureTemplate(randomPetCreatureIndex);
		if (!co)
		{
			sLog->outError("Creature with entry %d not exists.", randomPetCreatureIndex);
			return;
		}
		...
		if (!petInfo)
		{
			sLog->outError("Pet level info with entry %d and level %d not exists.", randomPetCreatureIndex, bot->getLevel());
			return;
		}
		...
		if (!pet->Create(guid, map, 0, co->Entry, pet_number))
		{
			delete pet;
			pet = NULL;
			sLog->outError("Pet with entry %d create failed.", co->Entry);
			return;
		}
		...
		sLog->outString("Bot %s: assign pet %d (%d level)", bot->GetName().c_str(), co->Entry, bot->getLevel());
...
void PlayerbotFactory::InitEquipment(bool incremental)
{
    ...
    //    if (ids.empty())
    //    {
    //        sLog->outString(  "%s: no items to equip for slot %d", bot->GetName(), slot);
    //        continue;
    //    }
...
void PlayerbotFactory::InitSecondEquipmentSet()
{
    ...
    for (map<uint32, vector<uint32> >::iterator i = items.begin(); i != items.end(); ++i)
    {
        ...
        for (int attempts = 0; attempts < 15; attempts++)
        {
			...
			bot->StoreNewItemInBestSlots(newItemId, 1);
        }
    }
}
...
void PlayerbotFactory::EnchantItem(Item* item)
{
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); ++id)
    {
        ...
        for (int j = 0; j < 3; ++j)
        {
			...
			SpellInfo const *pSpellInfo = sSpellMgr->GetSpellInfo(id);
            if (!item->IsFitToSpellRequirements(pSpellInfo))
                continue;
...
uint64 PlayerbotFactory::GetRandomBot()
{
    ...
    for (list<uint32>::iterator i = sPlayerbotAIConfig.randomBotAccounts.begin(); i != sPlayerbotAIConfig.randomBotAccounts.end(); i++)
    {
        ...
        do
        {
            ...
            uint64 guid = uint64(fields[0].GetUInt64());
...
void PlayerbotFactory::InitAmmo()
{
	...
	if (fields)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			bot->StoreNewItemInBestSlots(entry, 200);
		}
		bot->SetAmmo(entry);
	}
}
