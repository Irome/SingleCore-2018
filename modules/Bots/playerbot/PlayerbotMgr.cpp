...
void PlayerbotHolder::LogoutPlayerBot(uint64 guid)
{
    ...
    if (bot)
    {
		...
		sLog->outString("Bot %s logged out", bot->GetName().c_str());
        //bot->SaveToDB();
...
void PlayerbotHolder::OnBotLogin(Player * const bot)
{
    ...
    if (master)
    {
        uint64 masterGuid = master->GetGUID();
        if (master->GetGroup() &&
    ...
    if (sPlayerbotAIConfig.IsInRandomAccountList(account))
    {
		sLog->outString("%d/%d Bot %s logged in", playerBots.size(), sRandomPlayerbotMgr.GetMaxAllowedBotCount(), bot->GetName().c_str());
        if (sConfigMgr->GetBoolDefault("BeepAtStart", true) && playerBots.size() == sRandomPlayerbotMgr.GetMaxAllowedBotCount())
            printf("\a");
    }
	else sLog->outString("Bot %s logged in", bot->GetName().c_str());
}

string PlayerbotHolder::ProcessBotCommand(string cmd, uint64 guid, bool admin, uint32 masterAccountId, uint32 masterGuildId)
{
    if (!sPlayerbotAIConfig.enabled || guid == 0)
